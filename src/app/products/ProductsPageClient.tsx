  ...
  return (
    <main className="h-screen flex flex-row">
      ...
      <section className="flex-1 flex flex-col p-6 overflow-hidden">
        ...
        {/* ページネーション（省略付き） */}
        {meta && meta.pages > 1 && (
          <nav className="flex items-center justify-center gap-2 mt-4" aria-label="ページネーション">
            {/* 前へ */}
            {meta.page > 1 && (
              <Link
                href={{ pathname: "/products", query: { ...sp, page: String(meta.page - 1) } }}
                prefetch={false}
            ...
            {Array.from({ length: meta.pages }).map((_, i) => {
              ...
              if (pageNum === 1 || pageNum === meta.pages || Math.abs(pageNum - meta.page) <= 2) {
                return (
                  <Link
                    key={pageNum}
                    href={{ pathname: "/products", query: { ...sp, page: String(pageNum) } }}
                    prefetch={false}
                    className={`px-3 py-1 border rounded ${
                      meta.page === pageNum ? "bg-black text-white" : "hover:bg-gray-100"
                    }`}
                  >
                    {pageNum}
                  </Link>
              ...
              if ((pageNum === 2 && meta.page > 4) || (pageNum === meta.pages - 1 && meta.page < meta.pages - 3)) {
                return (
