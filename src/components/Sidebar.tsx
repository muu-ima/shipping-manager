...
export default function Sidebar({
    ...
    return (
        <aside
            aria-label="サイドバー"
            aria-expanded={open}
            className={['border-r bg-white/80 backdrop-blur overflow-y-auto transition-[width] duration-300',
                open ? 'w-72' : 'w-14',
            ].join(' ')}
        >
            {/*　ヘッダー(開閉トグル)　*/}
            <div className='flex h-11 items-center justify-between border-b px-2'>
                ...
                {/* 中央：タイトル（閉じ時はフェード） */}
                <span
                    className={`mx-2 select-none text-sm font-semibold transition-opacity ${open ? 'opacity-100' : 'opacity-0'
                        }`}
                >
                    フィルター
                </span>
